...
// GetOplogList 获取系统操作
func (a *audit) GetOplogList(name string, page, limit int) (data *OplogList, err error) {
	...
	// 定义返回的内容
	var (
		record []*model.LogOplog
		total  int64
	)

	// 获取菜单列表
	tx := global.MySQLClient.Model(&model.LogOplog{}).
		Where("username like ? OR client_ip like ? OR user_agent like ? OR endpoint like ?", "%"+name+"%", "%"+name+"%", "%"+name+"%", "%"+name+"%").
		Count(&total).
		Limit(limit).
		Offset(startSet).
		Order("id desc").
